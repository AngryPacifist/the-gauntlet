// ============================================================================
// Adrena Canonical Static Data
//
// Source-of-truth: synced from https://github.com/AdrenaFoundation/adrena-abi.
//
// Used by /admin/tradable-assets. Maps below mirror four authoritative sources
// in the abi repo:
...
//
// CRITICAL distinction: `pools_manifest.json:custodies[]` contains on-chain
// Custody account PDAs (e.g. Dk523LZ... for USDC custody), not SPL token
// mints. Engines match `position.token_account_mint === asset.mint` against
// SPL TOKEN MINTS, so the value stored in our `mint` field must come from
// src/lib.rs constants (USDC_MINT, BONK_MINT, JITO_MINT, WBTC_MINT),
// empirically equal to the live `/liquidity-info?pool_name=main-pool`
// response. Conflating these two would silently break ALL crypto scoring.
//
// ----------------------------------------------------------------------------
...
// (primary OHLC source is Pyth Benchmarks via ADRENA_TO_PYTH_SYMBOL in types.ts).
...
// curl https://datapi.adrena.trade/position?user_wallet=<wallet>&limit=1000
// across 732 real positions covering the 6 crypto assets. Findings:
//
...
//
// IMPLICATIONS:
// 1. SOL/BTC trading on Adrena routes through jitoSOL/WBTC custodies.
//    /position records position.token_account_mint = JITO_MINT (for SOL
//    trades) and WBTC_MINT (for BTC trades). To capture SOL trades in
//    scoring, asset.mint MUST be set to JITO_MINT. Same for BTC.
// 2. RWA positions use a SENTINEL system-program mint (11111…). They have
//    no real SPL token mint, so engine matching CANNOT use `mint`. Symbol
//    fallback is the only viable path. Since /position returns
//    "XAU"/"XAG"/"WTI" (uppercase), our uppercase asset symbols match
//    exactly.
...
// tournament's assetList; the engine would double-count every JitoSOL
// position once per asset entry. Same for BTC and WBTC. The frontend
// dropdown could flag this in a follow-up; for now, admin discretion.
// ----------------------------------------------------------------------------
...
//   DAO-bootstrapped):
// Synthetic custodies are virtual perpetuals with no underlying SPL token mint.
...
//
// EXPOSURE NOTE: surfaced under separate `synthetic_custody_mint` field,
// informational only. Engines do NOT match against this (they fall back to
// symbol matching for RWAs since `mint` stays undefined). See the empirical
// verification above: RWA positions return a system-program sentinel mint,
// not the synth PDA, so symbol fallback is the only matching path.
...
//
// API shape: RWA synthetic-custody mints surface under a SEPARATE optional
// field `synthetic_custody_mint` rather than the existing `mint` field.
// Empirical: RWA positions return position.token_account_mint =
// "11111111111111111111111111111111" (system program sentinel), NOT the
// synthetic-custody PDA. Engine matching against the synth PDA would fail
// every RWA position. Symbol fallback is the only viable path. The synth
// PDA stays in `synthetic_custody_mint` for admin UI display only.
// ----------------------------------------------------------------------------
export interface TradableAsset {
    ...
    mint?: string;
    /** Synthetic custody mint, commodities-pool RWAs only. Informational;
     *  engines do not match positions on this field (RWA positions return a
     *  system-program sentinel mint, so symbol fallback is the matching path). */
...
export function getTradableAssets(): TradableAsset[] {
    ...
    // main-pool: 6 crypto symbols. All have mints set:
