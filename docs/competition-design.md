...
2. Exclude positions with close exposure < $500 (`exit_size`, already in USD, with fallback to `entry_size`).
...
2. Admin calls `POST /api/admin/raffle/:id/compute`, which populates ticket counts and eligibility.
